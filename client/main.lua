        ...
        options = {
            {
                label = "Robbery Fence",
                icon = "fas fa-eye",
                event = "Cz-houserobberies:client:openblackmarket"
            },
        }
    })
end)

RegisterNetEvent("Cz-houserobberies:client:openblackmarket", function(data)
            ...
            if Config.Oxmenu then
                blackmarketmenu[#blackmarketmenu + 1] = {
                    icon = "nui://"..Config.Inventoryimagelink..QBCore.Shared.Items[v.item].image,
                     ...
                     event = "Cz-houserobberies:client:sellloot",
                     args = {
                        item = v.item,
                        min = v.minvalue,
                        max = v.maxvalue,
...
RegisterNetEvent("Cz-houserobberies:client:sellloot", function(data)
    local chance = math.random(1,100)
   
    if data.success >= chance then 
        TriggerServerEvent('Cz-houserobberies:server:sellloot', data.item)
    else
